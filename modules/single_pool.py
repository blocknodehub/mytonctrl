...
class SingleNominatorModule(PoolModule):
    ...
    def do_create_single_pool(self, pool_name, validator_wallet_name, owner_address):
    ...
    def new_single_pool(self, args):
        try:
            ...
            owner_address = args[2]
        except:
            color_print("{red}Bad args. Usage:{endc} new_single_pool <pool-name> <validator_wallet_name> <owner_address>")
            return
        self.do_create_single_pool(pool_name, validator_wallet_name, owner_address)
        color_print("new_single_pool - {green}OK{endc}")

    def do_activate_single_pool(self, pool, validator_wallet_name):
    ...
    def activate_single_pool(self, args):
        try:
            ...
            color_print("{red}Bad args. Usage:{endc} activate_single_pool <pool-name> <validator_wallet_name>")
            return
        ...
        if not os.path.isfile(pool.bocFilePath):
            ...
            return
        self.do_activate_single_pool(pool, validator_wallet_name)
